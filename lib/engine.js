...
const Engine = function () {
  ...
  const __isValidTest = function (test) {
    ...
    try {
      ...
      // test name can't be a reserved name: tollo-start, tollo-end
      ...
      if (_test.assert) {
        ...
        switch (_test.mode) {
          ...
          case settings.mode.PROMISE:
            _test.assert = engine.assert.equal
            break
          case settings.mode.CALLBACK:
            _test.assert = engine.assert.callback
            break
          case settings.mode.ASYNC:
            _test.assert = engine.assert.promise
            break
          case settings.mode.EVENT:
            _test.assert = engine.assert.event
            break
          case settings.mode.HTTP:
            _test.assert = engine.assert.http
            break
        }
      }
  ...
  const assert = {
    equal: function (result, input, output, sandbox) {
      return engine.promise((resolve, reject) => {
        engine.validate(result, { isExactly: output })
          ? resolve()
          : reject()
      })
    },
    mutation: function (result, input, output, sandbox) {
      return engine.promise((resolve, reject) => {
        engine.validate(input[0], { isExactly: output })
          ? resolve()
          : reject()
      })
    },
  ...
  Object.defineProperty(Engine.prototype, 'start', {
    get: () => {
      return __start
    },
    set: (f) => {
  ...
  Object.defineProperty(Engine.prototype, 'end', {
    get: () => {
      return __end
    },
    set: (f) => {
  ...
  Engine.prototype.mode = settings.mode

  Engine.prototype.assert = assert
  Engine.prototype.promise = promise
  Engine.prototype.callback = callback
  Engine.prototype.validate = checkv

  Engine.prototype.add = add
  Engine.prototype.bulk = bulk
  Engine.prototype.run = run
}

const engine = new Engine()
engine.global = {}

// custom wrapper of native js object
engine.Error = function (args = {}) {
  console.log(args)
  const _error = new Error()
  Object.assign(_error, args)
  return _error
}
