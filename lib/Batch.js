...
const Batch = function () {
  ...
  const add = function (test) {
    ...
    __orchestrator.add(test.name + '#start', _dependencies, (done) => {
      ...
      const _f = async () => {
        ...
        tap.test(_label, async (_tap) => {
          ...
          try {
            ...
            // it's always a good practice read source code
            // cheers, Simone
            // http://blog.cleancoder.com/uncle-bob/2017/03/16/DrCalvin.html
            try {
              log.info(settings.log.tag, _label, 'arrange')
              await _arrange(_case.input, _sandbox)
            } catch (e) {
              log.error(settings.log.tag, _label, 'arrange', log.v('err', e))
              _tap.fail(_label)
              return
            }

            try {
              log.info(settings.log.tag, _label, 'act')
              _case.result = await _act(_case.input)
            } catch (e) {
              _case.exception = e
            }

            log.info(settings.log.tag, _label, 'assert')
            if (_throw) {
    ...
    __orchestrator.add(test.name, _cases, (done) => {
  ...
  const run = function () {
    return new Promise((resolve, reject) => {
      ...
      __orchestrator.start(settings.tasks.end.name, (err) => {
        if (err) {
          reject(err)
          return
        }
