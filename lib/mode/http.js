  ...
  act: function (test) {
    return async function (input) {
      ...
      if (input) {
        ...
        if (input.files) {
          _options.formData = {}
          for (const _key in input.body) {
            _options.formData[_key] = JSON.stringify(input.body[_key])
          }

          for (const _name in input.files) {
            const _file = input.files[_name]
            // _options.formData[_name] = fs.createReadStream(_file.path)
            _options.formData[_name] = {
              value: fs.createReadStream(_file.path),
              options: {
                filename: _file.name,
        ...
        if (input.query) {
          _options.qs = input.query
          _options.qsStringifyOptions = { encode: false }
        }
      }
      ...
      if (output.body) {
        if (output.body.equal) {
          if (!_equal(_response.body, output.body.equal)) {
            log.info('tester-http', log.v('request', result.request), log.v('response', result.response))
            log.info('tester-http', 'body not equal', log.v('responde.body', _response.body), log.v('expected', output.body.equal))
            throw new Error('TOLLO_ASSERT_HTTP_BODY_EQUAL')
          }
        } else if (output.body.partial) {
          if (!_partial(_response.body, output.body.partial)) {
            log.info('tester-http', log.v('request', result.request), log.v('response', result.response))
            log.info('tester-http', 'body not partially equal', log.v('responde.body', _response.body), log.v('expected', output.body.partial))
