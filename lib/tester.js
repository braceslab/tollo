...
const Tester = function () {
  ...
  const assert = {
    equal: async function (result, input, output, sandbox) {
      if (!tester.validate(result, { isExactly: output })) {
        throw new Error('TOLLO_ASSERT_EQUAL_FAIL')
      }
    },
    mutation: async function (result, input, output, sandbox) {
      if (!tester.validate(input[0], { isExactly: output })) {
        throw new Error('TOLLO_ASSERT_MUTATION_FAIL')
      }
    },
    callback: async function (result, input, output) {
      if (output[0]) {
          // ? if output[0] instanceof Error reject output[0] ?
        throw new Error('TOLLO_ASSERT_CALLBACK_FAIL')
      }
    },
    /**
